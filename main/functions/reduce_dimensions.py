...
def smooth_columns(matrix: Union[np.ndarray, list], window_size: int) -> np.ndarray:
    ...
    smoothed_matrix = convolve1d(matrix, kernel, axis=0, mode='reflect')
    
    return smoothed_matrix

def reduce_dim(matrix: Union[np.ndarray, list], **kwargs) -> np.ndarray:
    """
    ...
    tsne_output = tsne.fit_transform(matrix)

    return tsne_output

def calculate_jumps(data: Union[np.ndarray, list], scan_rate: int = 0) -> Tuple[np.ndarray, np.ndarray]:
    """
        ...
        scan_rate : int (default = 0)
            An integer describing the amount of time inbetween each point in seconds/scan point. 
            Zero means no rate provided, result will be timepoints/jump vs. jumps/min.
    ...
    if not isinstance(scan_rate, int) or scan_rate < 0:
        raise ValueError("Scan rate must be a positive integer or zero.")
    ...
    if scan_rate == 0:
        ...
        duration_scan = (timepoints * scan_rate) / 60
        rate = n_jumps / duration_scan
